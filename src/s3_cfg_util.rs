...
impl CephKeysConfig {
    ...
    pub fn load() -> Result<Self> {
        ...
        let content = fs::read_to_string(&config_path).context(format!("Failed to read config file: {:?}", config_path))?;
        let config: CephKeysConfig = serde_yaml::from_str(&content).context("Failed to parse YAML config")?;
        Ok(config)
    }
    ...
    fn find_config_file() -> Result<PathBuf> {
        ...
        Err(anyhow!("Configuration file not found. Expected at ~/.config/s3cli/ceph_keys.yaml"))
    }
}
...
pub fn parse_bucket_url(bucket_url: &str) -> Result<String> {
    ...
    // 如果以 s3:// 开头，去掉前缀
    let bucket = if let Some(stripped) = bucket_url.strip_prefix("s3://") { stripped } else { bucket_url };
